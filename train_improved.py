...
def parse_args():
    ...
    p.add_argument("--plots", action="store_true", help="启用训练可视化绘图（显存/内存紧张时建议关闭）")
    return p.parse_args()
...
def _patch_bbox_loss_with_wiou(trainer, wiou):
    ...
    bbox_loss = None
    for loss_host_name in ("compute_loss", "loss"):
        loss_host = getattr(trainer, loss_host_name, None)
        if loss_host is not None and hasattr(loss_host, "bbox_loss"):
            bbox_loss = loss_host.bbox_loss
            break
    if bbox_loss is None:
        return False
